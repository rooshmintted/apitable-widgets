...
import { StyledMenuContainer, StyledSubMenu, StyledMenuItem, StyledMenuItemContent, StyledMenuItemArrow, StyledMenuShadow, StyledMenuItemExtra } from './styled';
...
const ContextMenuWrapper = (props) => {
    const { children, contextMenu, onClose, onShown, overlay, onClick, id, menuId, width = DEFAULT_MENU_WIDTH, menuOffset = [0, 0], menuSubSpaceHeight = 10, } = props;
    ...
    const cacheOverlay = useMemo(() => {
        if (!overlay || overlay.length === 0 || !contextMenuState.offset) {
            return null;
        }
        const cache = {};
        const calcCacheLevel = (list, level = 0) => {
            let sub = 0;
    ...
    const handleClick = (item, keyPath, e) => {
        if ((typeof item.disabled === 'function' && item.disabled(getExtraInfo(extraInfo))) ||
    ...
    // recursion
    const dfs = (source, results, index = 0) => {
        if (!results[index]) {
            results[index] = [];
        }
        ...
        for (let i = 0; i < filterHiddenSource.length; i++) {
            ...
            const { key, id, disabled = false, icon, label, arrow, children, groupId, extraElement, disabledTip } = newItem, rest = __rest(newItem, ["key", "id", "disabled", "icon", "label", "arrow", "children", "groupId", "extraElement", "disabledTip"]);
            ...
            let labelElement = (React.createElement(StyledMenuItem, Object.assign({}, rest, { key: id || key, onClick: (e) => handleClick(item, paths, e), onMouseEnter: () => handleMouseEnter(item, index), onTouchStart: () => handleMouseEnter(item, index), disabled: isDisabled, isGroup: isGroup, role: "menuitem" }),
                icon,
                React.createElement(StyledMenuItemContent, { variant: "body2", ellipsis: true }, typeof label === 'function' ? label(getExtraInfo(extraInfo)) : label),
                extraElement && React.createElement(StyledMenuItemExtra, null, extraElement),
                !extraElement && arrow && React.createElement(StyledMenuItemArrow, null, arrow)));
            if (isDisabled && disabledTip) {
                labelElement = (React.createElement(Tooltip, { title: disabledTip }, labelElement));
            }
            results[index].push({
                key,
                label: labelElement,
    ...
    const handler = React.useCallback((configs) => {
        if (!configs)
            return;
    ...
    const renderChildren = () => {
        ...
        return results.map((v) => {
            var _a;
            return (React.createElement(StyledSubMenu, { role: "menu", onScroll: handleScroll, key: `${(_a = v[0]) === null || _a === void 0 ? void 0 : _a.key}-ul` },
                v.map((item) => item.label),
                React.createElement(StyledMenuShadow, null)));
        });
    };
    ...
    useEffect(() => {
        ...
        for (let i = 0; i < childList.length; i++) {
            ...
            const lastElement = child.lastElementChild;
            if (!parentKey) {
        ...
        ` : '';
            ...
            const parentContainer = childList[i - 1];
            ...
            let childStartX = childList[0].offsetLeft + preParentWidthSum + menuOffset[0];
    ...
    return createPortal(React.createElement(StyledMenuContainer, { id: id, style: style, ref: menuRef }, renderChildren()), document.body);
};
