    ...
    if (resizeMsg) {
        ...
        updateTasks = tasks.map(task => {
            if (id === task.id) {
                ...
                return Object.assign(Object.assign({}, task), formatData);
            }
            return task;
        });
    }
    const rowTasks = updateTasks.filter(task => (!isAfter(date2Day(task.startDate || task.endDate), endDate) && !isAfter(startDate, task.endDate || task.startDate)) ||
        // The start date is greater than the end date. Exception task processing
        (isAfter(task.startDate, task.endDate) && !isBefore(task.startDate, startDate) && !isBefore(endDate, task.startDate))).map(task => {
