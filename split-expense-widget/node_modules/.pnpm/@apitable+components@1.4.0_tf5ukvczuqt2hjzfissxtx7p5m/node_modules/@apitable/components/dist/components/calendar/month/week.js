...
export const WeekBase = (props) => {
    ...
    return (React.createElement("div", { className: "week-row", key: weekLevel, style: { height: rowHeight + 'px' } },
        React.createElement("div", { className: "row-bg" }, week.map((weekItem, idx) => {
            ...
            const dayContent = (React.createElement(DaySpan, { className: "day-value" }, dayValue));
            return (React.createElement(DayDiv, { key: `${m}-${day}`, className: classNames('day', {
                    weekend: [0, 6].includes((idx + 1) % 7),
                    curMonth: m === month,
                    today: isToday
                }) }, Drop ? React.createElement(Drop, { children: dayContent, date: currDay, update: update, disabled: disabled }) : dayContent));
        })),
        takeLevels.map((level, levelIdx) => {
            return (React.createElement("div", { className: "level-row", key: levelIdx }, level.map((levelItem, levelItemIdx) => (React.createElement(Task, { key: levelItemIdx, levelItem: levelItem, levelIdx: levelIdx })))));
        }),
        more.map((m, mIndex) => {
            if (m <= MAX_LEVEL) {
                return null;
            ...
            return (React.createElement(MoreTask, { key: mIndex, mIndex: mIndex, curDay: curDay, moreTasks: moreTasks[mIndex], takeLevelLen: takeLevels.length }));
        })));
};
