...
export const DropdownSelect = (props) => {
    ...
    const renderOptionList = ({ toggle }) => {
        ...
        dropdownRender || React.createElement(ListDeprecate, { onClick: (_e, index) => {
            ...
            } : undefined, noDataTip: noDataTip, triggerInfo: triggerInfo, autoHeight: true }, afterFilterOptions.filter(Boolean).map((item, index) => {
            return (React.createElement(OptionItem, { onClick: () => {
                    setVisible(false);
                    toggle();
                    onSelected && onSelected(afterFilterOptions[index], index);
    ...
    return React.createElement(React.Fragment, null,
        ...
        React.createElement(StyledDropdown, { onVisibleChange: (visible) => {
                setVisible(visible);
                ...
                disabled,
            }, trigger: React.createElement("div", { style: triggerStyle },
                React.createElement(WrapperTooltip, { wrapper: Boolean(disabledTip && disabled), tip: disabledTip },
                    React.createElement(StyledSelectTrigger, { onClick: () => {
                            triggerClick();
                        }, style: triggerStyle, className: triggerCls, tabIndex: -1, ref: containerRef, disabled: Boolean(disabled), focus: visible, "data-name": 'select' },
                        ...
                        !hiddenArrow && React.createElement(StyledArrowIcon, { rotated: visible },
                            React.createElement(ChevronDownOutlined, { color: disabled ? Color(theme.color.black[500]).alpha(0.5).hsl().string() : theme.color.black[500] }))))) }, renderOptionList));
};
const Option = ListDeprecate.Item;
DropdownSelect.Option = Option;
export function OptionItem({ item, currentIndex, value, keyword, className, onClick, iconClassName, }) {
    ...
    return React.createElement(OptionOutside, Object.assign({ currentIndex: currentIndex, setRef: ref, tabIndex: isActive ? 0 : -1, active: isActive, selected: isSelected, className: className, id: item.value }, item, itemProps),
        React.createElement(SelectItem, { iconClassName: iconClassName, item: item, renderValue: _renderValue, isChecked: value === (item === null || item === void 0 ? void 0 : item.value) }, !keyword ? null : React.createElement(Highlighter, { highlightClassName: hightLightCls.toString(), searchWords: [keyword], autoEscape: true, textToHighlight: item.label })));
}
