 ...
 * @param gap shade gap
 */
export const getNextShadeColor = (color, gap) => {
    const hueShade = colorHexHueShadeMap[color.toUpperCase()];
    if (hueShade) {
        const [hue, currentShade] = hueShade;
        const currentLevel = shadeLevel.findIndex(shade => shade === currentShade);
        ...
        if (nextShade)
            return allColors[hue][nextShade];
        return color;
    }
 ...
 */
export const rgba2hex = (foregroundColor, backgroundColor = '#FFFFFF') => {
 ...
 * @param opacity
 */
export const editRgbaOpacity = (rgbaColor, opacity) => {
